...
def hexagon(x, y, scale=1.0, m_sqrt3=None, extent=None, patch=True, angle=None, **kwargs):
 ...
 if patch:
  ...
  polygons = PolyCollection([polygon], offsets=offsets, offset_position="data",
              transOffset=mtrans.IdentityTransform(), **kwargs)
...
def get_radius(x, y):
 ...
 radius = dist[dist > 0.][0]
 return radius

def readMegaraPosTable(fits, extname='FIBERS', ext=None, table=True, lkeys=None, angle=None, ref_x=0.0, ref_y=0.0):
...
class RSSViewer:
 ...
 def __init__(self,name_fits,ptable,fitscom=None,dfilter='filters/',norm='sqrt', 
		default_filter='Halpha_KPNO-NOAO',fo=1.0,fc=1.0,extension=False,
		palpha=0.95, plw=0.1,plc='k',clw=1,cc='r',cf=False,ca=0.8,
		slw=2,sf=False,sa=0.9,colorbar=False,fits=False,txt=True,
		integrated=True,mval=0.0,individual=False,wlim=None,flim=None,
...
# Create Figure 1 (Spaxel Viewer) ---------------------------------
  ...
  self.p = self.getPatches(cmap=plt.cm.gray_r, norm=rnorm(self.norm), 
...
# DS9-like dynamic range of colormap
 ...
 def readPositionTable(self, extension=False):
  if not extension:
   ...
   self.id, self.x, self.y = np.loadtxt(self.ptable,unpack=True,usecols=(0,1,2),skiprows=1)
  else:
   try:
  ...
  if len(iname) == 0:
   ...
   if (self.txt is not True and self.fits is not True) or (self.integrated is not True and self.individual is not True):
    ...
    if self.individual:
 	...
 	'Spaxel (ID): ' + str(item)]
      infohd = [['RSSVWR_1',infotxt[0]],['RSSVWR_2',infotxt[1]]]
      save_spec(self.wl,self.dat[item],fname+'_'+str(item).zfill(self.nsfm),
		fits=self.fits,hd=self.hd,txt=self.txt,infohd=infohd,infotxt=infotxt)
    print('Files Saved')
 ...
 def xyLimits(self):
  ...
  if len(lm) == 0: pass
   ...
   if not isinstance(lm, (tuple, list)) or len(lm) != 4:
    print ('Flux limits should be a tuple or list of 4 items: ex. --> -6.5, None, -6, 6  [xmin, xmax, ymin, ymax]')
    return
   ...
   if ymax is not None:
    self.ymax = ymax
   print('*** XY limits set to: (%s, %s, %s, %s) ***' % (self.xmin, self.xmax, self.ymin, self.ymax))
 ...
 def getPatch(self, idf, **kwargs):
  ...
  if self.ft == 'H':
   patch = hexagon(self.x[idf], self.y[idf], scale=self.hex_scale, angle=self.angle, **kwargs)[0]
  return patch
    ...
    if event.button == 1:
     ...
     if event.key == 'd':
      ...
      if idf not in self.list:
       self.pat.append(self.getPatch(idf, color=self.cc, alpha=self.ca, lw=self.clw, fill=self.cf))
 ...
 def GetSpectraInfo(self,event):
  ...
  if str(idl) != self.sint:
   sp = self.getPatch(int(idl)-1, color=col, ec=col, lw=self.slw, fill=self.sf, alpha=self.sa)
   self.cir.append(sp)
 ...
 def IntFilter(self,ifi,fil,lm,dat,pasb=False,verb=True,dfil='',dl=None,center=False,remove_cont=True):
 ...
 def WindowManager(self):
  ...
  if not plt.fignum_exists(3):
   ...
   def update(val):
    self.plw = plw.val; self.p.set_linewidth(self.plw)
    ...
    self.fig.canvas.draw()
   plw.on_changed(update); pap.on_changed(update)

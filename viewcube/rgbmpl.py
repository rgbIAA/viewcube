...
def ilinear(inputArray, scale_min=None, scale_max=None,verb=False):
    ...
    """		
    if verb is True:  print ("Scale : linear")
...
def isqrt(inputArray, scale_min=None, scale_max=None,verb=False):
    ...
    if verb is True:  print ("Scale : sqrt")
...
# ----------------------------------------------------------------------------
#
def ilog(inputArray, scale_min=None, scale_max=None, oiraf=True, z1=1., 
	z2 = 1000., verb=False):
   ...
   if verb is True:  print ("Scale : log")
   ...
   if oiraf is False:
    ...
    imageData[imageData == 0.0] = nan
    imageData = (imageData-scale_min)*a + z1
    imageData[indices0] = z1
    imageData[indices2] = z2
    factor = numpy.log10(z2-z1)
    imageData = numpy.log10(imageData)/factor
   
   imageData[isnan(imageData)] = 0.0
   imageData[isinf(imageData)] = 0.0
...
def iasinh(inputArray, scale_min=None, scale_max=None, non_linear=2.0,verb=False):
    ...
    if verb is True:  print ("Scale : asinh")
...
# ----------------------------------------------------------------------------
#
def isfun(input,scale='linear',vmin=None,vmax=None,zsc=True,oiraf=True,z1=1.,
	z2=1000.,nlf=2.0,omap=False,verb=False):
 '''
##############################################################################
# --------------------------- Subrutina ISFUN --------------------------------
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
 ...
 if scale == 'linear':
  ...
  dat = ilog(iarray,scale_min=vmin,scale_max=vmax,oiraf=oiraf,
		z1=z1,z2=z2,verb=verb)
...
# ----------------------------------------------------------------------------
 '''
 def __init__(self,scale='linear',vmin=None,vmax=None,zsc=None,
	z1=1.,z2=1000.,exponent=2.,vmid=None,clip=False):
...
# RUMPL
   ...
   if self.scale == 'linear':
    ...
    raise Exception("Unknown scale in 'rnorm': %s" %
                          self.scale)
 ...
 def inverse(self, value):
  ...
  if self.scale == 'linear':
   ...
   val = self.midpoint * \
      ...
      a = (z2 - z1)/(vmax - vmin)
      factor = ma.log10(z2 - z1)
      val = vmin + (ma.power(10.,(val*factor)) - z1)/a
...
# ----------------------------------------------------------------------------
 ...
 def ChangeDynamicRangeColorMap(self,event):
  ...
  #ax = event.canvas.figure.gca()
   ...
   if self.opf is True:
    if self.vmin < self.rmin: self.vmin = self.rmin
    if self.vmax > self.rmax: self.vmax = self.rmax
 ...
 # Request for another scale, linear, logarithmic etc.
   ...
   if len(nval) == 0: 
    ...
    try:
     if type(eval(nval)) in (numpy.float, numpy.int):
      ...
      print ('Min/Max = %8.4e / %8.4e  ||  Set to %8.4e / %8.4e' %\
...
def lsfiles(strfind,dir='.',path=False):
 ...
 if (path is not False) and (dir != '.'):
   return [dir + item for item in
                        sorted(fnmatch.filter(os.listdir(dir),strfind))]
# ----------------------------------------------------------------------------
...
# ----------------------------- Subrutina SAVESPEC ---------------------------
# ----------------------------------------------------------------------------
def SaveSpec(wl,spec,name,fits=False,txt=True,infotxt=None,
	hd=None,infohd=None,endtxt='.txt',fmt='%-10.2f %14.6e'):
 if txt:
  f = open(name+endtxt,'w')
  if infotxt is not None:
   if type(infotxt) is not list: infotxt = [infotxt]
   for item in infotxt:
    f.write('# ' + item + '\n')
...
def GetLambdaLimits(wl,pt=0.05,wlim=None):
 ...
 if wlim is not None:
  if type(wlim) not in [list,tuple] or len(wlim) != 2:
   ...
   if wlimmin is not None: min = wlimmin
   if wlimmax is not None: max = wlimmax
 range = abs(max - min)
...
def IntFilter(ifi,fil,lm,dat,pasb=False,verb=True,dfil='',dl=None):
 lf,ff = loadtxt(dfil+fil[ifi],unpack=True)
 if dl is not None: lf = lf + dl
 if verb is True:
  print ('Selected Filter: ' + fil[ifi])
 fff = interp(lm,lf,ff)
 if pasb is True:
  return trapz(lm*fff*dat,lm,axis=1)/trapz(fff*lm,lm), fff
 else:
  return trapz(lm*fff*dat,lm,axis=1)/trapz(fff*lm,lm)
# ----------------------------------------------------------------------------

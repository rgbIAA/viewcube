...
# PPAK special cases
 if len(x) == 331 or len(x) == 993: yfbar = 1.3
 if len(x) == 382:  xfbar = 1.3
...
def GetLambdaLimits(wl, pt=0.05, wlim=None):
 ...
 if wlim is not None:
  if type(wlim) not in [list,tuple] or len(wlim) != 2:
   ...
   if wlimmin is not None: wmin = wlimmin
   if wlimmax is not None: wmax = wlimmax
 range = abs(wmax - wmin)
...
def PRectangle(x,y,r):
 ...
 if isinstance(x,(int,float,np.int,np.float)):
  ...
  xv = x[:,numpy.newaxis] + numpy.array([0.,0.,r,r])
  yv = y[:,numpy.newaxis] + numpy.array([0.,r,r,0.])
 return xv, yv
# ------------------------------------------------------------------------
...
# Basic Info
  ...
  self.fobj = LoadFits(self.name_fits,exdata=exdata,exhdr=exhdr,exwave=exwave,exflag=exflag,exerror=exerror,specaxis=specaxis,**kwargs)
  self.K = self.fobj.K
  ...
  if masked and self.flag is not None:
   self.dat = np.ma.array(self.dat, mask=np.logical_or(self.flag > 0, ~np.isfinite(self.dat)))
  ...
  if self.syn is not None:
   if LooseVersion(self.pversion) < LooseVersion('2.0.0'):
    ...
    if self.K.hasSegmentationMask:
     self.K.zres = (self.K.f_obs-self.K.f_syn) * self.K.flux_unit / self.K.fobs_norm
     self.K.tres = (self.K.flux_unit * (self.K.f_obs-self.K.f_syn)).sum(axis=1) / self.K.fobs_norm.sum()
     self.zones  = self.K.spatialize(np.arange(1, self.K.segmentationMask.shape[0] + 1), extensive=False)
     ...
     self.K.res  = (self.K.f_obs-self.K.f_syn).filled(0.0) * self.K.flux_unit / self.K.fobs_norm
     self.K.fres = (self.K.f_obs-self.K.f_syn).filled(0.0) * self.K.flux_unit
     self.K.zres = (self.K.f_obs-self.K.f_syn).filled(0.0)[:, ~self.K.synthImageMask] * self.K.flux_unit / self.K.fobs_norm[~self.K.synthImageMask]
     self.K.tres = ((self.K.flux_unit * (self.K.f_obs-self.K.f_syn))[:, ~self.K.synthImageMask]).sum(axis=1) / (self.K.fobs_norm[~self.K.synthImageMask]).sum()
     self.zones  = np.zeros(self.K.synthImageMask.shape)
     self.zones[~self.K.synthImageMask] = np.arange(1, (~self.K.synthImageMask).sum() + 1)
  # Lambda
  if naxes == 2:
  ...
  if naxes == 3:
   ...
   try:
    ...
    self.crpix1 = int(self.xx/2.)
    self.crpix2 = int(self.yy/2.)
...
# Read Position Table
  if self.ptable is not None:
   ...
   self.id, self.x, self.y, self.flag = loadtxt(self.ptable,unpack=True,skiprows=1)
...
# DS9-like dynamic range of colormap
  if self.iclm: self.icmp = IntColorMap(self.p)
  plt.show()
 ...
 def onselect(self,eclick, erelease):
  ...
  #x,y=self.pix2res(tile(arange(ix,fx),fy-iy),repeat(arange(iy,fy),fx-ix))
 ...
 def SaveFile(self):
  ...
  print('%10s = %-5s | %10s = %-5s' % ('integrated',str(self.integrated),'individual',str(self.individual)))
  ...
  if len(iname) == 0:
   ...
   if iname.find(',') != -1:
    ...
    for item in iname.split(',')[1:]:
     if item.lower().find('fits') != -1: self.fits = eval(item.split('=')[1])
     if item.lower().find('txt') != -1:  self.txt  = eval(item.split('=')[1])
     if item.lower().find('integrated') != -1:  self.integrated  = eval(item.split('=')[1])
     if item.lower().find('individual') != -1:  self.individual  = eval(item.split('=')[1])
   else:
    fname = iname
   if (self.txt is not True and self.fits is not True) or (self.integrated is not True and self.individual is not True):
    print ("Nothing to save")
   else:
    if self.root == fname: stint = '_int'
    else:                  stint = ''
    if self.integrated:
     infotxt = ['Integrated Spectra extracted from: "' + self.name_fits + '"', 
	'Sum of Spaxels (ID): ' + ' | '.join([str(id) for id in self.list])]
     infohd = [['3DVWR_1',infotxt[0]],['3DVWR_2',infotxt[1]],['CRVAL1',self.crval],['CDELT1',self.cdelt]]
     self.intspec = array([self.dat[:,y,x] for x,y in self.list]).sum(0)
     save_spec(self.wl,self.intspec,fname+stint,fits=self.fits,txt=self.txt,
		hd=self.hd,infohd=infohd,infotxt=infotxt)
    if self.individual:
     for item in self.list:
      infotxt = ['Spectra extracted from: "' + self.name_fits + '"', 
 	'Spaxel (ID): ' + str(item)]
      infohd = [['3DVWR_1',infotxt[0]],['3DVWR_2',infotxt[1]],['CRVAL1',self.crval],['CDELT1',self.cdelt]]
      x,y = item; strl = ('%'+str(self.fx)+'i_%'+str(self.fy)+'i') % (x,y)
      save_spec(self.wl,self.dat[:,y,x],fname+'_'+strl,fits=self.fits,hd=self.hd,
			txt=self.txt,infohd=infohd,infotxt=infotxt)
    print ('Files Saved')
 ...
 def RestWave(self,event):
  if event.key == 'z' and self.orig_wl_rest is not None:
   if self.wrest and self.velocity is not None:
    print('Rest Wavelength (redshift = %8.5f | velocity = %7.1f km/s)' % (self.redshift,self.velocity))
 ...
 def SynthSpec(self,event):
  if event.key == 'p':
   ...
   if self.syncom and self.syn is not None:
    ...
    self.dat2 = self.syn
    if self.ix is not None and self.iy is not None:
     self.ax2.plot(self.wl2,self.dat2[:,self.iy,self.ix],c=self.ccom,lw=self.lcom)
    self.fig2.canvas.draw()
   if not self.syncom and self.syn is not None:
    self.fitscom = None
    if self.ix is not None and self.iy is not None:
     self.ax2.cla()
     self.ax2.plot(self.wl,self.dat[:,self.iy,self.ix],c=self.cspec,lw=self.lspec)
     self.updatePassBand()
     self.fig2.canvas.draw()
 ...
 def BoxFilter(self,wini=4500.,wend=5000.,dl=1.0):
  lf = np.arange(wini-dl,wend+dl,dl)
  ff = np.ones(lf.shape)
  ff[0] = 0.0; ff[-1] = 0.0
  return lf, ff

 def IntFilter(self,ifi,fil,lm,dat,pasb=False,verb=True,dfil='',dl=None,center=False,remove_cont=False):
 ...
 def SpectraViewer(self,event):
  ...
  if event.inaxes and tb.mode == '' and not self.RS.active:
   ...
   if (0 <= ix < self.xx) and (0 <= iy < self.yy) and not isinstance(self.p._A[iy,ix], np.ma.core.MaskedConstant):
    self.fig.canvas.toolbar.set_message('(RA,DEC) = (%4.2f,%4.2f) | (x,y) = (%2i,%2i) | z = %.2f' % (x,y,ix,iy,self.p._A[iy,ix]))
 ...
 def PlotSpec(self):
  ...
  self.ax2.plot(self.wl,self.spec,c=self.cspec,lw=self.lspec,label=self.slabel,picker=True)
 ...
 def PlotPycassoIntSpec(self):
  ...
  self.pband = self.ax2.fill_between(self.wl,self.fff*self.spec.max()*self.fp,color='g',alpha=0.25)
  self.ax2.set_xlim((self.awlmin,self.awlmax))
  self.ax2.set_ylim((self.fmin,self.fmax))
 ...
 def GetSpectraInfo(self,event):
  if self.view_pintspec:
   return
  if len(self.cir) > 0:
   [item[0].set_visible(False) for item in self.cir] #[item.remove() for item in self.cir]
 ...
 def PassBandPress(self,event):
  ...
  if not self.pband.contains(event)[0]: return
  tb = plt.get_current_fig_manager().toolbar
# Inicializar self.dl para que cuando se clicka en el mismo sitio no haga nada
 ...
 def ResidualViewer(self,event):
  ...
  if not event.inaxes and tb.mode == '' and event.button == 1:
   ...
   self.ax5.plot(self.wl,self.K.tres)
   self.ax5.set_ylim(-0.1,0.1)
   self.ax5.text(1.,-0.36,'Integrated',weight='bold',
	transform=self.ax5.transAxes,ha='right',multialignment='right')
   self.fig4.canvas.draw()
  ...
  if event.key == 'x' and len(self.list) > 0:
   ...
   if self.fitspec_mode % 2 == 0:
    if plt.get_backend() != 'TkAgg':
     print ('*** You need to set backend to "TkAgg" if you want to use PyRAF interactive fitting ***')
    else:
    ...
    sp = pyspeckit.Spectrum(xarr=self.wl,data=self.spec,error=self.espec,header=self.hd)
    sp.plotter()
 ...
 def WindowManager(self):
  ...
  if not plt.fignum_exists(3):
   ...
   # Circle Properties ------------------------
   cs = gridspec.GridSpec(3,3,left=0.2,bottom=0.55,top=0.7,wspace=0.7,hspace=0.5,right=0.9)
   ...
   ss = gridspec.GridSpec(3,3,left=0.2,bottom=0.3,top=0.45,wspace=0.7,hspace=0.5,right=0.9)
   ...
   svp = gridspec.GridSpec(3,2,left=0.2,bottom=0.05,top=0.2,wspace=0.7,hspace=0.5,right=0.9)
   ...
   def update(val):
    ...
    self.palpha = pap.val; self.p.set_alpha(self.palpha)
    self.clw = clw.val; self.ca = cap.val
    self.slw = slw.val; self.sa = sap.val
   ...
   clw.on_changed(update); cap.on_changed(update)
   slw.on_changed(update); sap.on_changed(update)

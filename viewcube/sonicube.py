...
class SoniCube(object):
    ...
    def autoencoding (self, spectrum):
        ...
        redim_spectrum = np.zeros((1, self.spectrum_dim))   # We need to adapt the dimensions to the autoencoder input
    ...
    def coord_converter (self, y, x):
        ...
        azimuth = (90 - math.degrees(math.atan2((y-self.y_ref), (x-self.x_ref))) % 360)   #Calculates azimuth
        radial_distance = math.sqrt((y-self.y_ref)**2 + (x-self.x_ref)**2)                #Calculates x,y distance to x_ref, y_ref

        coordinates = [(0, 0), (0, self.y_tot), (self.x_tot, 0), (self.x_tot, self.y_tot)]  #Calculates max_distance to normalize
        max_distance = 0
    ...
    def ponderating (self, frequencies):    #returns the A-weighted amplification factor to compensate loudness
        ...
        for freq in frequencies:
            if freq != 0:
                ...
                norm_weight = (weight-(-80))/((1.27134)-(-80))      #MAX-MIN normalization for 24KHz bandwith
                if (norm_weight) != 0:

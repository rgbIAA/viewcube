...
# -------------------------------------------------------------------------------------
def save_spec(wl, spec, name, fits=False, txt=True, infotxt=None, hd=None,
              infohd=None, endtxt='.txt', fmt='%-10.2f %14.6e'):
    if txt:
...
class LoadFits:
 ...
 def __init__(self, name, exdata=None, exflag=None, exerror=None, exerw=None, exfibc=None, exflat=None,
	      exwave=None, exsensf=None, exhdr=0, error_file=None, flag_file=None, wave_file=None,
              sensf_file=None, hdisp='DISPAXIS', hcrval='CRVAL', hcdelt='CDELT', crval=None, cdelt=None,
              haxis=True, specaxis=None, hvel=None, z=None, velocity=None, guess=True, flip2disp=True,
              rss=False, f2e=None, bflag=None, c=299792.458, dist_pc=None, filters=None, filter_props=None,
              filter_mode=None, sensf=True, units=None, ivar=False, verbo=True):
  '''
    ...
    if 'EXTNAME' in list(hdu[i].header.keys()):
     ...
     if isinstance(self.instrument, str) and 'WEAVE' in self.instrument:
      ...
      if hdu[i].header['EXTNAME'].split()[0].endswith('_IVAR'):
       ...
       self.error = 1. / np.sqrt(hdu[i].data)
  ...
  if self.isivar and self.error is not None:
   self.error = 1. / np.sqrt(self.error)

  if isinstance(self.instrument, str) and 'WEAVE' in self.instrument and self.sensfunc is not None and sensf:
  ...
  if self.cdelt is None and self.wave is None and self.crval is not None: 
   ...
   # For MUSE
   if ('CTYPE3' in list(self.hdr.keys())) and (self.hdr['CTYPE3'].find('AWAV') != -1) and ('CD3_3' in list(self.hdr.keys())):
  ...
  if self.flag is None and bflag is not None:
   if verbo: print('*** No FLAG found!! We create one and set all to %s!! ***' % bflag)
